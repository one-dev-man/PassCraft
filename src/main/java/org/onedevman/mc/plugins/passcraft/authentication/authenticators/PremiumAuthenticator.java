...
import org.onedevman.mc.plugins.passcraft.apis.MojangAPI;
...
//                    }
                    ...
                    if(_this.enabled() && MojangAPI.user.isPremium(player_name)) {
                        ...
                        ReflectCraft.getNativeServer().setUsesAuthentication(true);
            ...
            if(event.getPacketType().equals(PacketType.Login.Server.ENCRYPTION_BEGIN)) {
                try {
                    ...
                    if(_this.authqueue.size() == 0)
                        ReflectCraft.getNativeServer().setUsesAuthentication(false);
                } catch (Exception e) {
                    throw new RuntimeException(e);
                }
            }
