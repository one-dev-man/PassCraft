            ...
            if(event.entity() instanceof Player player) {
                try {
                    if(this.authmanager.authenticators.password().isRegistered(player))
                        Notify.chat(
                            player,
                            this.authmanager.configParser.getString(Locale.CONFIGPATHS.MESSAGES.ALREADY_PASSWORD_REGISTERED),
            ...
            if(event.entity() instanceof Player player) {
                try {
                    if(!AuthManager.isLogged(this.authmanager.authqueue, player))
                        ...
                        Notify.chat(
                            player,
                            this.authmanager.configParser.getString(Locale.CONFIGPATHS.MESSAGES.DISCORD_ALREADY_LINKED),
                            Sound.BLOCK_NOTE_BLOCK_BASS
                        );
                    else
